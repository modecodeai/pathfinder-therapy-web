import { describe, expect, it } from 'vitest';
import { createDefaultTaxationConfig } from '../src/emdr/types/emdrTaxation';
import {
  estimateWorkingMemoryLoad,
  reduceTaxationOneStep,
  resolveActiveModifiers,
  taxationConfigPatchToRoom,
} from '../src/emdr/engine/taxationPresets';
import {
  resolveEffectiveElapsed,
  resolveTaxationColour,
  resolveSpeedScale,
} from '../src/emdr/engine/taxationEngine';
import {
  TaxationMotionRuntime,
  reversalTraversalRange,
  patternTraversalRange,
} from '../src/emdr/engine/taxationMotion';
import { taxationPointAt } from '../src/emdr/engine/taxationTrajectory';
import { createDefaultRoomState, roomStateToTaxationConfig } from '../src/types/room';
import { passesFromElapsed } from '../src/features/visual/blsEngine';
import { generateTask, makeEasier, makeHarder } from '../src/emdr/engine/cognitiveTasks';
import { getScriptById } from '../src/emdr/help/library';

describe('Working Memory Taxation — Standard path unchanged', () => {
  it('default room taxation mode is standard', () => {
    const s = createDefaultRoomState();
    expect(s.taxationMode).toBe('standard');
    expect(roomStateToTaxationConfig(s).mode).toBe('standard');
  });

  it('resolveEffectiveElapsed is identity in standard mode', () => {
    const cfg = createDefaultTaxationConfig();
    expect(resolveEffectiveElapsed(cfg, 0)).toBe(0);
    expect(resolveEffectiveElapsed(cfg, 1234)).toBe(1234);
    expect(resolveEffectiveElapsed(cfg, 5000)).toBe(5000);
  });

  it('pass counting helpers still match fixed cycle duration', () => {
    const cycleMs = 1000;
    expect(passesFromElapsed(0, cycleMs)).toBe(0);
    expect(passesFromElapsed(999, cycleMs)).toBe(0);
    expect(passesFromElapsed(1000, cycleMs)).toBe(1);
  });

  it('standard mode keeps base colour', () => {
    const cfg = createDefaultTaxationConfig();
    const r = resolveTaxationColour(cfg, '#14B8A6', 12);
    expect(r.colour).toBe('#14B8A6');
    expect(r.changed).toBe(false);
  });
});

describe('Variable speed taxation', () => {
  it('produces eased non-identity elapsed while staying bounded', () => {
    const cfg = {
      ...createDefaultTaxationConfig(),
      mode: 'variable-speed' as const,
      variableSpeedPreset: 'medium' as const,
      seed: 42,
    };
    const e1 = resolveEffectiveElapsed(cfg, 2000);
    const e2 = resolveEffectiveElapsed(cfg, 4000);
    expect(e1).toBeGreaterThan(0);
    expect(e2).toBeGreaterThan(e1);
    expect(e2).toBeLessThan(4000 * 2);
    const scale = resolveSpeedScale(cfg, 1500);
    expect(scale).toBeGreaterThanOrEqual(0.5);
    expect(scale).toBeLessThanOrEqual(1.6);
  });
});

describe('Colour taxation', () => {
  it('colour-shift rotates deterministically', () => {
    const cfg = {
      ...createDefaultTaxationConfig(),
      mode: 'colour-shift' as const,
      colourShiftInterval: 2 as const,
      colourPalette: ['#FFFFFF', '#3B82F6', '#22C55E'],
      seed: 7,
    };
    const a = resolveTaxationColour(cfg, '#14B8A6', 0);
    const b = resolveTaxationColour(cfg, '#14B8A6', 2);
    expect(b.colour).toBe(resolveTaxationColour(cfg, '#14B8A6', 2).colour);
    expect(['#FFFFFF', '#3B82F6', '#22C55E']).toContain(a.colour);
    expect(b.colour).not.toBe(a.colour);
  });
});

describe('Direction Shift + Pattern Switch + Chaos + Custom', () => {
  it('resolveActiveModifiers enables direction shift', () => {
    const mods = resolveActiveModifiers({
      ...createDefaultTaxationConfig(),
      mode: 'direction-shift',
      directionShiftRate: 'high',
    });
    expect(mods.directionShift).toBe(true);
    expect(mods.patternSwitch).toBe(false);
  });

  it('pattern switch requires multiple trajectories', () => {
    const one = resolveActiveModifiers({
      ...createDefaultTaxationConfig(),
      mode: 'pattern-switch',
      enabledTrajectories: ['horizontal'],
    });
    expect(one.patternSwitch).toBe(false);
    const many = resolveActiveModifiers({
      ...createDefaultTaxationConfig(),
      mode: 'pattern-switch',
      enabledTrajectories: ['horizontal', 'vertical', 'wide-arc'],
    });
    expect(many.patternSwitch).toBe(true);
  });

  it('chaos 3 enables concurrent modifiers', () => {
    const mods = resolveActiveModifiers({
      ...createDefaultTaxationConfig(),
      mode: 'chaos',
      chaosLevel: 3,
    });
    expect(mods.variableSpeed).toBe(true);
    expect(mods.directionShift).toBe(true);
    expect(mods.patternSwitch).toBe(true);
    expect(mods.randomColour).toBe(true);
    expect(mods.trajectories.length).toBeGreaterThan(3);
  });

  it('custom intensity alters speed preset and frequency mapping', () => {
    const low = resolveActiveModifiers({
      ...createDefaultTaxationConfig(),
      mode: 'custom',
      customIntensity: 2,
      customChangeFrequency: 2,
      customToggles: {
        ...createDefaultTaxationConfig().customToggles,
        variableSpeed: true,
        earlyDirectionReversal: true,
        horizontal: true,
      },
    });
    const high = resolveActiveModifiers({
      ...createDefaultTaxationConfig(),
      mode: 'custom',
      customIntensity: 10,
      customChangeFrequency: 10,
      customToggles: {
        ...createDefaultTaxationConfig().customToggles,
        variableSpeed: true,
        earlyDirectionReversal: true,
        patternSwitching: true,
        horizontal: true,
        vertical: true,
      },
    });
    expect(low.variableSpeedPreset).toBe('low');
    expect(high.variableSpeedPreset).toBe('high');
    expect(high.changeFrequency).toBeGreaterThan(low.changeFrequency);
  });

  it('motion runtime advances continuously without NaN', () => {
    const runtime = new TaxationMotionRuntime();
    const cfg = {
      ...createDefaultTaxationConfig(),
      mode: 'direction-shift' as const,
      directionShiftRate: 'high' as const,
      seed: 123,
    };
    let lastX = -1;
    for (let t = 0; t < 5000; t += 16) {
      const frame = runtime.tick({
        wallElapsedMs: t,
        cycleDurationMs: 1400,
        config: cfg,
        baseColour: '#14B8A6',
        travelWidth: 0.85,
        verticalPosition: 0.5,
        midline: 'up',
        preferredTrajectory: 'horizontal',
      });
      expect(Number.isFinite(frame.x)).toBe(true);
      expect(Number.isFinite(frame.y)).toBe(true);
      expect(frame.x).toBeGreaterThanOrEqual(0);
      expect(frame.x).toBeLessThanOrEqual(1);
      if (lastX >= 0) {
        expect(Math.abs(frame.x - lastX)).toBeLessThan(0.35);
      }
      lastX = frame.x;
    }
    expect(runtime.tick({
      wallElapsedMs: 8000,
      cycleDurationMs: 1400,
      config: cfg,
      baseColour: '#14B8A6',
      travelWidth: 0.85,
      verticalPosition: 0.5,
      midline: 'up',
    }).passFloor).toBeGreaterThan(0);
  });

  it('pattern switch motion changes trajectory over time', () => {
    const runtime = new TaxationMotionRuntime();
    const cfg = {
      ...createDefaultTaxationConfig(),
      mode: 'pattern-switch' as const,
      patternSwitchRate: 'high' as const,
      enabledTrajectories: ['horizontal', 'vertical', 'diagonal-a'] as const,
      seed: 55,
    };
    const seen = new Set<string>();
    for (let t = 0; t < 20000; t += 16) {
      const frame = runtime.tick({
        wallElapsedMs: t,
        cycleDurationMs: 1000,
        config: { ...cfg, enabledTrajectories: [...cfg.enabledTrajectories] },
        baseColour: '#fff',
        travelWidth: 0.85,
        verticalPosition: 0.5,
        midline: 'up',
      });
      seen.add(frame.trajectory);
    }
    expect(seen.size).toBeGreaterThan(1);
  });

  it('trajectory points stay in field for all taxation trajectories', () => {
    const trajs = [
      'horizontal',
      'vertical',
      'diagonal-a',
      'diagonal-b',
      'wide-arc',
      'figure-eight',
    ] as const;
    for (const traj of trajs) {
      for (const u of [0, 0.25, 0.5, 0.75, 1]) {
        const p = taxationPointAt(traj, u, {
          travelWidth: 0.85,
          verticalPosition: 0.5,
          midline: 'up',
        });
        expect(p.x).toBeGreaterThanOrEqual(0);
        expect(p.x).toBeLessThanOrEqual(1);
        expect(p.y).toBeGreaterThanOrEqual(0);
        expect(p.y).toBeLessThanOrEqual(1);
      }
    }
  });

  it('reversal and pattern ranges match clinical bands', () => {
    expect(reversalTraversalRange('low', 5, false)[0]).toBeGreaterThanOrEqual(6);
    expect(reversalTraversalRange('high', 5, false)[1]).toBeLessThanOrEqual(4);
    expect(patternTraversalRange('random', 5, false)[0]).toBe(2);
    expect(patternTraversalRange('random', 5, false)[1]).toBe(12);
  });
});

describe('Working memory load indicator', () => {
  it('standard → standard; chaos 3 → high', () => {
    expect(estimateWorkingMemoryLoad(createDefaultTaxationConfig())).toBe('standard');
    expect(
      estimateWorkingMemoryLoad({
        ...createDefaultTaxationConfig(),
        mode: 'chaos',
        chaosLevel: 3,
      }),
    ).toBe('high');
  });

  it('maps reduce/increase patches onto RoomState keys', () => {
    const cfg = {
      ...createDefaultTaxationConfig(),
      mode: 'variable-speed' as const,
      variableSpeedPreset: 'high' as const,
    };
    const patch = taxationConfigPatchToRoom(reduceTaxationOneStep(cfg));
    expect(patch.taxationVariableSpeedPreset).toBe('medium');
  });
});

describe('Cognitive tasks & help', () => {
  it('generate / easier / harder stay coherent', () => {
    const t = generateTask('numerical');
    expect(t.modality).toBe('numerical');
    const harder = makeHarder(t);
    expect(harder.difficulty).toBeGreaterThanOrEqual(t.difficulty);
    const easier = makeEasier(harder);
    expect(easier.difficulty).toBeLessThanOrEqual(harder.difficulty);
  });

  it('help articles exist', () => {
    expect(getScriptById('wmt-overview')?.title).toMatch(/Working Memory Taxation/);
    expect(getScriptById('wmt-phase-guide')).toBeTruthy();
  });
});

describe('no coming-soon labels in mode list', () => {
  it('TAXATION_MODE_ORDER has all eight modes', async () => {
    const { TAXATION_MODE_ORDER, TAXATION_MODE_LABELS } = await import(
      '../src/emdr/types/emdrTaxation'
    );
    expect(TAXATION_MODE_ORDER).toEqual([
      'standard',
      'variable-speed',
      'colour-shift',
      'random-colour',
      'direction-shift',
      'pattern-switch',
      'chaos',
      'custom',
    ]);
    for (const m of TAXATION_MODE_ORDER) {
      expect(TAXATION_MODE_LABELS[m]).not.toMatch(/coming soon/i);
    }
  });
});
